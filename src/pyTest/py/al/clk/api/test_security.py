...
def _apply(scheme, credentials, kwargs=None):
    from al.clk.api.security import apply
...
def test_apply_oauth2_client_credentials(monkeypatch):
    from al.clk.api import security as sec
...
def test_enumerate_schemes_from_spec():
    from al.clk.api.security import enumerate_schemes
...
def test_schemes_for_operation_prefers_op_over_top_level():
    from al.clk.api.security import schemes_for_operation
    spec = {
...
def test_registry_exposes_scheme_dropdown_for_secured_op(monkeypatch):
    ...
    dropdown."""
    import al.clk.api.presets as presets_mod
    from al.clk.api.presets import Preset
    from al.clk.api.loader import load_spec
    from al.clk.api.registry import _register_preset, NODE_CLASS_MAPPINGS
