...
# Load `api.env` if present — keeps tool homes + port defaults out of
# individual tests. Prefer `python-dotenv` (proper parser) and fall
# back to a tiny in-repo parser so the suite still runs without the
# extra dep.
_ENV_FILE = REPO / "api.env"
try:
    from dotenv import load_dotenv as _load
    _load(_ENV_FILE, override=False)
    ...
    if _ENV_FILE.is_file():
        for _line in _ENV_FILE.read_text().splitlines():
            ...
            os.environ.setdefault(
                _k.strip(),
                os.path.expandvars(_v.strip().strip('"').strip("'")),
            )
...
_WM_HOST = os.environ.get("WIREMOCK_HOST", "127.0.0.1")
_WM_PORT = os.environ.get("WIREMOCK_PORT", "8089")
WIREMOCK_URL = os.environ.get("WIREMOCK_URL", f"http://{_WM_HOST}:{_WM_PORT}")
SKIP_BOOTSTRAP = os.environ.get("SKIP_BOOTSTRAP") == "1"

sys.path.insert(0, str(REPO / os.environ.get("PY_MAIN", "src/pyMain/py")))
