...
Callable from pytest (via conftest's wiremock_base fixture) or as a CLI:
    ...
    python -m tests.bootstrap --check           # report state, no install
    python -m tests.bootstrap --install-jbang   # install SDKMAN + jbang
    python -m tests.bootstrap --fetch-spec      # pull/refresh OpenAPI cache
